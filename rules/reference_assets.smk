if REFERENCE_ASSET_TARGET_FILES:
    ...
    rule export_reference_hard_mask_bed:
        input:
            hard_mask=REFERENCE_HARD_MASK_TSV,
            metadata=RUN_METADATA
        output:
            hard_bed=REFERENCE_HARD_MASK_BED
        log:
            project_path("logs", "reference_assets", "export_hard_mask_bed.log")
        params:
            python_bin=config["biosoft"]["python"],
            script=SCRIPT_BUILD_BIN_ANNOTATIONS,
            script_action=SCRIPT_BUILD_BIN_ANNOTATIONS_ACTION
        ...
        shell:
            r"""
            mkdir -p "$(dirname {output.hard_bed})" "$(dirname {log})"
            (
                echo "=== PIPELINE AUDIT ==="
            ...
            {params.python_bin:q} {params.script:q} {params.script_action:q} \
                hard-mask-bed \
                --hard-mask-tsv {input.hard_mask:q} \
                --output-bed {output.hard_bed:q} \
                >> {log:q} 2>&1
            """
    ...
    rule build_frozen_reference_package:
        ...
        shell:
            ...
            {params.python_bin:q} {params.script:q} {params.script_action:q} \
                ...
                --asset-files {input.atomic_bins:q} {input.analysis_bins:q} {input.qc_bins:q} {input.atomic_annotations:q} {input.analysis_annotations:q} {input.qc_annotations:q} {input.hard_mask:q} {input.hard_bed:q} {input.soft_mask:q} {input.dynamic_mask:q} {input.combined_mask:q} \
