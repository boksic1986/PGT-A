...
CNV_PREDICT_BAM_QC_CFG = CNV_QC_CFG.get("predict_bam_qc", {})
CNV_PREDICT_BAM_QC_DIR = str(Path(CNV_QC_DIR) / "predict_bam_qc")
CNV_PREDICT_BAM_QC_FLAGSTAT = str(Path(CNV_PREDICT_BAM_QC_DIR) / "{sample}.samtools.flagstat.txt")
CNV_PREDICT_BAM_QC_IDXSTATS = str(Path(CNV_PREDICT_BAM_QC_DIR) / "{sample}.samtools.idxstats.tsv")
CNV_PREDICT_BAM_QC_TSV = str(Path(CNV_PREDICT_BAM_QC_DIR) / "{sample}.bam_qc.tsv")
CNV_PREDICT_BAM_QC_JSON = str(Path(CNV_PREDICT_BAM_QC_DIR) / "{sample}.bam_qc.json")
CNV_PREDICT_BAM_QC_SUMMARY_TSV = str(Path(CNV_PREDICT_BAM_QC_DIR) / "predict_bam_qc_summary.tsv")
CNV_PREDICT_BAM_QC_SUMMARY_JSON = str(Path(CNV_PREDICT_BAM_QC_DIR) / "predict_bam_qc_summary.json")
...
CNV_SAMPLE_REPORT_QC_DIR = str(Path(CNV_POSTPROCESS_DIR) / "sample_report_qc")
CNV_SAMPLE_REPORT_QC_TSV = str(Path(CNV_SAMPLE_REPORT_QC_DIR) / "sample_report_qc.tsv")
CNV_SAMPLE_REPORT_QC_JSON = str(Path(CNV_SAMPLE_REPORT_QC_DIR) / "sample_report_qc.json")
