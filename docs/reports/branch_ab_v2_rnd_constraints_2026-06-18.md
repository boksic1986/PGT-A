...
- 2026-06-20 SCA evidence review updated the interpretation constraint:
  PAR / XY-homology evidence is annotation, consistency, and artifact-risk
  context, not a primary whole-SCA call seed. Final-oriented Branch S V2 must
  use sex-aware X/Y dosage with X non-PAR and Y non-PAR kept separate, and must
  apply the expected ploidy for the current sex call. See
...
Branch B passes a report-contract iteration only if:

- `cnv_report` explicitly consumes the agreed Branch B evidence/disposition
  output rather than the legacy hard-filter kept-event table alone;
- the report states the Branch B evidence version, reference ID, binsize,
  mask/preprocess contract, negative/background source, and whether the
  background is `N0`, cross-fit, reference-cohort, N1 shadow, or unknown;
- reportable events are derived from Branch A candidates plus Branch B
  dispositions/evidence, not from the legacy Branch B kept-event path alone;
- Branch B still produces no B-only final report event;
- WisecondorX predict, Branch A, Branch B, Branch S, benchmark/evaluation,
  and report all use the same reference/config contract;
- Branch S output states whether SCA evidence is final-reportable, review-only,
  or development-only; the current unvalidated Branch S output remains
  review/development-only;
- missing refmap, calibration-null, matched-background, or Branch S evidence is
  not treated as benign support.

The 2026-06-15 five-sample report must **not** be released as the old
current-workflow report. The previous `cnv_summary.*` outputs may be retained
only as a legacy baseline / comparison artifact. Any report released after this
decision must be generated from the revised Branch A/reference plus Branch B/S
...
- New reference ID has two rerun levels:
  - audit level: rerun WisecondorX predict and Branch A candidates; current
    Branch B/S/report outputs are optional diagnostic snapshots only;
  - report-facing level: rerun fixed Branch B, Branch S, evaluation, benchmark,
    and report under the same reference/config contract.
...
- The 2026-06-15 five-sample legacy outputs remain exploratory comparison
  artifacts, not releasable reports and not locked validation proof.

## Current Recommendation

Proceed with a Branch A / reference reset first, then continue Branch B as a
candidate-refinement and reporting-evidence layer.

This corrects the previous over-emphasis on immediate Branch B V2 report-contract
implementation. The current V2/N1 shadow implementation is useful for design
review, but it is not mature enough to be the main report path by itself.
...
candidate samples. In P1/P2, rerun WisecondorX predict and Branch A candidates
against Y1-Y8, H1-H6, H7-H16, and the 2026-06-15 shadow-report set. Branch B/S
and report outputs at this stage are diagnostic snapshots unless their updated
contracts have passed G3/G5.

Branch B remains necessary, but its role is narrower and more explicit:

- refine Branch A candidates without suppressing sensitivity;
- keep weak positives such as mosaic or focal events visible as review;
- add LOH / UPD / mosaic and copy-number-like annotations when evidence is
  available;
- explain region, noise, and batch risk;
- reduce false-positive burden only after no-FN validation.

CNVpro-inspired analysis is a reference direction and an independent comparison
track. It should be used to evaluate whether GC/RC/waviness, p/q-arm and
acrocentric handling, copy-number/mosaic proxies, and sex-aware X/Y logic improve
interpretation. It must not replace WisecondorX predict as Branch A.

Formal `N0` / cross-fit background remains the clean promotion path, but it is
not an immediate blocker for development because the current sample size makes a
complete N0 design difficult. Until a reliable N0/cross-fit set exists,
background-derived evidence must be reported as limited/reference-context or
unknown, not as hard PASS/artifact proof.

## 2026-06-20 Design Addendum

Focused feasibility design:

`docs/reports/branch_b_v2_reference_background_and_sca_design_2026-06-20.md`

Current gate/phase control file:

`docs/reports/branch_ab_v2_gate_phase_plan_2026-06-20.md`

The gate/phase file is the execution-order source of truth for this R&D cycle.
Older Phase 1-5 implementation reports remain historical evidence, not the
current phase order.

This addendum updates the next Branch B / Branch S direction:

- audit and stabilize the candidate reference cohort before freezing Branch A;
- use reference-cohort background only as limited context, not as formal N0;
- keep formal N0/cross-fit background as a later promotion gate rather than the
  immediate development blocker;
- rerun WisecondorX predict and Branch A after every reference change; rerun
  fixed Branch B, Branch S, evaluation, benchmark, and report before any
  report-facing release or promotion decision;
- upgrade Branch B evidence toward LOH / UPD / mosaic / CN-like annotations and
  CNVpro-inspired consistency checks;
- move Branch S toward a final reportable SCA workflow, while keeping current
  unvalidated Branch S output clearly marked as development/shadow evidence.

### Current R&D Go / No-Go Status

Current status: **GO for Branch A/reference reset and Branch B evidence redesign;
NO-GO for legacy current-workflow report release; NO-GO for unvalidated hard V2
promotion; GO for SCA workflow finalization as a separate validation track**.

The R&D direction can continue because:

- the current architecture still preserves WisecondorX-first candidate
  generation;
- Branch A remains the recall-preserving signal generator;
- Branch B remains candidate refinement and review-burden control;
- Branch B can incorporate CNVpro-inspired evidence and LOH / UPD / mosaic
  annotations without creating B-only candidate events;
- reference-cohort evidence can provide limited context while N0/cross-fit
  remains a later promotion standard;
- Branch S has a defined development path toward reportable SCA classification,
  but the current Branch S output is not yet final;
- SCA interpretation has been corrected to a conservative sex-aware dosage
  model: PAR / XY-homology is annotation and consistency/artifact context, not
  a primary whole-SCA call seed.

The direction is not yet promotable because:

- the current H-augmented reference is still a shadow reference and requires
  whole-cohort abnormal-sample audit before Branch A can be fixed;
- current Branch B V2/N1 evidence is based on limited background and cannot
  justify hard filtering or PASS/CONFIRMED labels;
- no locked independent `N0` / cross-fit negative background is available, and
  this limitation must be explicit rather than solved by overfitting thresholds;
- Branch S lacks locked SCA truth coverage for X gain, XXY, XYY, Y loss, mosaic
  SCA, and clean XX/XY negatives across batches;
- the 2026-06-15 five-sample set has already been used during exploratory
  review, so it cannot serve as a clean validation set.

### Next Stage Target: Reference-Fixed Branch A Plus Branch B/S Evidence Report

The next operational target is not to promote the current Branch B V2 prototype.
It is to produce a report package only after:

1. the candidate reference cohort is audited and a named Branch A/reference
   contract is fixed;
2. WisecondorX predict and Branch A candidates are regenerated under that
   reference;
3. Branch B adds candidate-level evidence, LOH / UPD / mosaic and CN-like
   annotations where available, and CNVpro-inspired consistency checks;
4. Branch S produces sex-aware SCA evidence that is either validated for report
   use or explicitly marked as non-final development evidence;
5. Y1-Y8 and H1-H6 no-FN checks are rerun before the 2026-06-15 five-sample
   report is handed out.

The old current-workflow Branch A+B `cnv_summary.*` output is not an acceptable
forward release artifact.

Legacy comparison output already exists and can be used only for comparison:
...
Legacy sample-level report burden:

| sample | sex | Branch B kept | reportable |
|---|---|---:|---:|
| `JZ26125843-56-56` | XX | 0 | 0 |
| `JZ26125844-59-59` | XX | 3 | 3 |
| `JZ26125845-60-60` | XY | 26 | 26 |
| `JZ26125846-61-61` | XY | 3 | 3 |
| `JZ26125847-62-62` | XY | 1 | 1 |

Before handing out the five-sample report, the report package must state:

- reference/config contract used by WisecondorX predict;
- Branch A candidate count and top signal per sample;
- Branch B evidence version, candidate disposition counts, LOH / UPD / mosaic
  annotation status, and per-sample report burden;
- whether each event has only unknown/limited background or stronger
  independent/cross-fit support;
- whether CNVpro-inspired checks agree, disagree, or are missing;
- Branch S/SCA status, including whether the SCA decision is final-reportable or
  still development-only;
- that samples with zero Branch B reportable events still may have Branch A
  signals and are not clean validation negatives;
- that five-sample report output is not a locked benchmark and not proof of hard
  Branch B promotion.

Immediate implementation implication:

- first generate a reference-sample audit for the current H-augmented shadow ref
  and any expanded ref candidate set;
- only after the Branch A/reference contract is fixed, update report inputs so
  `cnv_report` consumes the agreed Branch B evidence/disposition table rather
  than the legacy hard-filter kept-event path alone;
- extend Branch B evidence with LOH / UPD / mosaic and CNVpro-inspired
  consistency fields before using it as the report evidence table;
- define the SCA report boundary separately from autosomal/segmental Branch B;
- rerun remote unit tests, Snakemake dry-runs, and formal Y/H/0615 evaluations
  on `fengxian`.
